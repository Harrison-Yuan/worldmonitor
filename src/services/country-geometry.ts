...
const COUNTRY_NAMES_ZH: Record<string, string> = {
  ...
  CM: '喀麦隆', CA: '加拿大', CF: '中非共和国', TD: '乍得', CL: '智利',
  ...
  NI: '尼加拉瓜', NE: '尼日尔', NG: '尼日利亚', MK: '北马其顿', NO: '挪威',
  OM: '阿曼', PK: '巴基斯坦', PW: '帕劳', PS: '巴勒斯坦', PA: '巴拿马',
  PG: '巴布亚新几内亚', PY: '巴拉圭', PE: '秘鲁', PH: '菲律宾', PL: '波兰',
  PT: '葡萄牙', QA: '卡塔尔', RO: '罗马尼亚', RU: '俄罗斯', RW: '卢旺达',
  KN: '圣基茨和尼维斯', LC: '圣卢西亚', VC: '圣文森特和格林纳丁斯', WS: '萨摩亚', SM: '圣马力诺',
  ST: '圣多美和普林西比', SA: '沙特阿拉伯', SN: '塞内加尔', RS: '塞尔维亚', SC: '塞舌尔',
  SL: '塞拉利昂', SG: '新加坡', SK: '斯洛伐克', SI: '斯洛文尼亚', SB: '所罗门群岛',
  SO: '索马里', ZA: '南非', SS: '南苏丹', ES: '西班牙', LK: '斯里兰卡',
  SD: '苏丹', SR: '苏里南', SE: '瑞典', CH: '瑞士', SY: '叙利亚',
  TW: '台湾', TJ: '塔吉克斯坦', TZ: '坦桑尼亚', TH: '泰国', TL: '东帝汶',
  TG: '多哥', TO: '汤加', TT: '特立尼达和多巴哥', TN: '突尼斯', TR: '土耳其',
  TM: '土库曼斯坦', TV: '图瓦卢', UG: '乌干达', UA: '乌克兰', AE: '阿联酋',
  GB: '英国', US: '美国', UY: '乌拉圭', UZ: '乌兹别克斯坦', VU: '瓦努阿图',
  VA: '梵蒂冈', VE: '委内瑞拉', VN: '越南', YE: '也门', ZM: '赞比亚', ZW: '津巴布韦',
  // Additional common names
  EU: '欧盟', UN: '联合国', NATO: '北约',
  SAUDI: '沙特', UAE: '阿联酋', UK: '英国', USA: '美国',
};

/**
 * Get the display name for a country code, localized to the current language.
 * Falls back to the English name from GeoJSON, then the raw code.
 */
export function getCountryDisplayName(code: string): string {
  const upper = code.toUpperCase();
  const lang = getCurrentLanguage();
  if (lang === 'zh') {
    const zhName = COUNTRY_NAMES_ZH[upper];
    if (zhName) return zhName;
  }
  return getCountryNameByCode(code) || upper;
}
